...
const OakRoute = (props: Props) => {
  ...
  const authenticate = async (type: string, redirect = true) => {
    ...
    const accessToken = getSessionValue(`winmeet-access_token`);
    const refreshToken = getSessionValue(`winmeet-refresh_token`);
    if (accessToken && refreshToken) {
      httpPost(
        ...
        .then((response) => {
          if (response.status === 200) {
            ...
            if (response.data.access_token) {
              ...
              setSessionValue(`winmeet-access_token`, newAccessToken);
            }
            dispatch(
              addAuth({
                isAuth: true,
        ...
        .catch((error: any) => {
          removeSessionValue(`winmeet-access_token`);
          removeSessionValue(`winmeet-refresh_token`);
